    ...
    def Activated(self):
        ...
        if job.SplitOutput:
            for index, slist in enumerate(postlist):
                ...
                result = self.exportObjectsWith(slist, job, partname)
                if result is None:
                    success = False
                else:
                    gcode += result
            ...
            gcode = self.exportObjectsWith(finalpostlist, job, filename)
            success = gcode is not None
            #(success, rc, filename) = self.exportObjectsWith(finalpostlist, job)
