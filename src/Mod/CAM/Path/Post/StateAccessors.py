...
MachineConfiguration dataclass format.
...
# State can be either dict or typed configuration
State = Union[Dict[str, Any], "MachineConfiguration"]

# Lazy import to avoid circular dependency while maintaining performance
# This will be set on first use to avoid circular import at module load time
_MachineConfiguration = None

def _get_machine_config_class():
    """Get MachineConfiguration class, importing on first call."""
    global _MachineConfiguration
    if _MachineConfiguration is None:
        from Path.Post.Processor import MachineConfiguration
...
def get_output_comments(state: State) -> bool:
    ...
    if isinstance(state, _get_machine_config_class()):
...
def set_motion_mode(state: State, mode: str) -> None:
    ...
    if isinstance(state, _get_machine_config_class()):
        ...
        raise TypeError("Cannot mutate MachineConfiguration - use dict for mutable state")
